    ...
    <div className="min-h-screen bg-card text-foreground">
      ...
      <section className="pt-28 pb-16 px-6">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-[56px] font-bold tracking-tight leading-[1.1] mb-5 text-foreground">
            One app to replace
            <br />them all.
          </h1>
      ...
      <section className="px-6 pb-20">
        <div className="max-w-5xl mx-auto">
          <div className="rounded-xl border border-border bg-card overflow-hidden shadow-xl shadow-gray-200/60">
            {/* Tab bar */}
            <div className="flex items-center gap-0 px-4 border-b border-border bg-muted/60">
              ...
              {['List', 'Board', 'Calendar'].map((tab, i) => (
                <button key={tab} className={`px-3 py-2.5 text-xs font-medium border-b-2 ${i === 1 ? 'border-[#7b68ee] text-[#7b68ee]' : 'border-transparent text-muted-foreground'}`}>
                  {tab}
                </button>
              ))}
            </div>
            {/* Board preview */}
            <div className="flex h-[320px] bg-muted/40">
              {/* Sidebar mini */}
              <div className="w-44 border-r border-border bg-card p-2.5 space-y-0.5 hidden md:block">
                <div className="flex items-center gap-2 mb-3 px-2">
                  <div className="h-5 w-5 rounded bg-gradient-to-br from-[#7b68ee] to-[#a78bfa] flex items-center justify-center">
                    <Brain className="h-3 w-3 text-white" />
                  </div>
                  <span className="font-semibold text-[11px] text-foreground">BrainForge</span>
                </div>
                {[
                  ...
                  <div key={item.label} className={`px-2 py-1 rounded text-[11px] ${item.active ? 'bg-[#7b68ee]/8 text-[#7b68ee] font-medium' : 'text-muted-foreground'}`}>
                    {item.label}
                  </div>
                ))}
              </div>
                ...
                {[
                  ...
                  <div key={col.title} className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2 px-1">
                      ...
                      <span className="text-[11px] font-medium text-foreground/80">{col.title}</span>
                      <span className="text-[10px] text-muted-foreground bg-muted px-1.5 rounded">{col.count}</span>
                    </div>
                    <div className="space-y-1.5">
                      {col.items.map((item) => (
                        <div key={item} className="bg-card rounded-lg border border-border p-2.5 shadow-sm">
                          <p className="text-[11px] text-foreground/80 font-medium">{item}</p>
