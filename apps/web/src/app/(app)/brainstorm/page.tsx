    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Collaborate, discuss, and visualize ideas</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {MODES.map(({ value, label, icon: Icon, color, desc, gradient }) => (
          <button
            ...
            className={cn(
              'bg-gradient-to-br border border-border rounded-2xl p-5 text-center',
              'hover:shadow-md hover:border-border hover:-translate-y-0.5 transition-all duration-200',
              'disabled:opacity-60 disabled:cursor-wait',
              gradient,
            )}
            onClick={() => handleAutoCreate(value)}
          >
            <div className="h-12 w-12 rounded-xl mx-auto mb-3 flex items-center justify-center bg-card shadow-sm relative" style={{ color }}>
              {creatingMode === value ? (
        ...
        <div className="space-y-2">
          {sessions?.data?.map((s: any) => {
            ...
            return (
              <div
                key={s.id}
                className="bg-card border border-border rounded-xl p-4 flex items-center gap-3 cursor-pointer hover:shadow-md hover:border-border transition-all duration-200 group"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-xs text-muted-foreground">{s._count?.messages || 0} messages</span>
          ...
          {!sessions?.data?.length && (
            <div className="text-center py-16 bg-muted/50 rounded-2xl border border-dashed border-border">
              ...
              <p className="text-xs text-muted-foreground/60 mt-1">Click a mode above to start your first brainstorm session</p>
            </div>
          )}
        </div>
      </div>
