...
const PROVIDERS = [
  {
    value: 'OPENAI', label: 'OpenAI', color: '#10a37f', bg: '#ecfdf5',
  ...
  {
    value: 'CLAUDE', label: 'Anthropic', color: '#d4a574', bg: '#fef7ed',
  ...
  {
    value: 'GEMINI', label: 'Google Gemini', color: '#4285f4', bg: '#eff6ff',
  ...
  {
    value: 'GROQ', label: 'Groq', color: '#f55036', bg: '#fef2f2',
  ...
  {
    value: 'OPENROUTER', label: 'OpenRouter', color: '#6366f1', bg: '#eef2ff',
              ...
              <>
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    className="border-gray-200 focus:border-[#7b68ee] rounded-xl h-10 font-mono text-sm"
                  />
                </div>
        ...
        <DialogContent className="bg-white sm:max-w-[700px] max-h-[85vh] rounded-2xl flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto space-y-1 pr-1 -mr-1">
            {filteredModels.map(({ provider, model: m, providerConfig }) => {
              ...
              return (
                <div key={`${provider}-${m.id}`} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-gray-50 transition-colors">
                  <div
                    className="h-7 w-7 rounded-lg flex items-center justify-center text-sm shrink-0"
                    style={{ backgroundColor: providerConfig?.bg || '#f3f4f6' }}
                  >
                    {providerConfig?.icon || '⚪'}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      {isFree && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-green-50 text-green-600 border border-green-100">FREE</span>}
                      {m.description && <span className="text-[10px] text-gray-400 hidden sm:inline">{m.description}</span>}
                    </div>
            ...
            {filteredModels.length === 0 && (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-400">No models found for &quot;{modelSearch}&quot;</p>
              </div>
            )}
          </div>
        </DialogContent>
